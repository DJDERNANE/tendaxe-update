...
<div class="my-2">
    <div class="bg-white px-3 pt-3 border rounded">
        <a href="{{ route('detail', $offre) }}" style="text-decoration: none; color: inherit; width: 100%">
            ...
            <div class="row">
                ...
                <div class="col-3 col-md-2 text-right">
                    ...
                    @else
                        ...
                        @else
                            <img class="rounded-circle" src="{{ asset('storage/logo/' . $offre->adminetab->logo) }}"
                                width="50px">
                        @endif
                    @endif
        ...
        <div class="mb-2 d-flex">
            <span class="mx-auto mt-2 text-15"> <img src="{{ asset('img/icons/play.png') }}">
                {{ $offre->date_pub }}</span>
            <span class="mx-auto mt-2 text-15"> <img src="{{ asset('img/icons/stop.png') }}">
                {{ $offre->date_limit }}</span>
            <div>
                <a href="{{ route('detail', $offre) }}" class="btn btn-sm btn-primary px-3 ml-auto">Detail</a>
                {{-- <a href="{{ route('participer', $offre) }}" class="btn btn-sm btn-primary px-3 ml-auto p-0 participer">Participer
                    <i class="bi bi-microsoft-teams" style="font-size: 20px"></i></a> --}}
                @if (Auth::check() && Auth::user()->type_user === 'content')
                    <a href="{{ route('rep.offers.edit', $offre) }}"
                ...
                @if (Auth::check() && Auth::user()->type_user === 'admin' && $offre->trashed())
                    <button class="btn btn-sm btn-secondary" data-toggle="modal" data-target="#restoremodal"
                        data-id="{{ $offre->id }}">Restore</button>
                @endif
                @if (Auth::check() && Auth::user()->type_user === 'admin' && $offre->etat === 'pending')
                    <button class="btn btn-sm btn-success" data-toggle="modal" data-target="#acceptmodal"
                        data-id="{{ $offre->id }}">accepter</button>
                @endif
            </div>
        </div>
