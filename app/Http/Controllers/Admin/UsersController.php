...
class UsersController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($request->wilaya_user && $request->wilaya_user !== 'all'){
            $users = $users->where([
               
                ['wilaya', $request->wilaya_user],
            ]);
        }

        if($request->abonnement && $request->abonnement !== 'all'){
            $today = Carbon::today();

            switch ($request->abonnement) {
                case 'free_active':
                    $users = $users->whereHas('abonnement_latest', function ($query) use ($today) {
                        $query->where('nom_abonnement', 'gratuit')
                              ->where('date_fin', '>=', $today);
                    });
                    break;

                case 'free_past':
                    // Filter users with free and expired subscriptions
                    $users = $users->whereHas('abonnement_latest', function ($query) use ($today) {
                        $query->where('nom_abonnement', 'gratuit')
                            ->where('date_fin', '<', $today);
                    });
                ...
                case 'abnm_active':
                    // Filter users with paid and active subscriptions
                    $users = $users->whereHas('abonnement_latest', function ($query) use ($today) {
                        $query->where('nom_abonnement', 'gratuit')
                            ->where('date_fin', '>=', $today);
                    });
                ...
                case 'abnm_past':
                    // Filter users with paid and expired subscriptions
                    $users = $users->whereHas('abonnement_latest', function ($query) use ($today) {
                        $query->where('nom_abonnement', '<>', 'gratuit')
                            ->where('date_fin', '<', $today);
                    });
        ...
        if ($request->start_date && $request->end_date) {
            $users = $users->whereHas('abonnement_latest', function ($query) use ($request) {
                $query->whereBetween('date_fin', [$request->start_date, $request->end_date]);
            });
        }
        ...
        $wilayas = User::distinct()->pluck('wilaya');
        $today = Carbon::today();
